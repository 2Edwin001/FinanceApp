...
import { DollarSign, BarChart2, Target, CalendarDays, Scale, FileDown } from 'lucide-react'
...
import PDFReportContent from '../components/dashboard/PDFReportContent'
...
export default function Dashboard() {
  ...
  const { getTotalIncomeByMonth } = useIncomes()
  ...
  const incomeTotal = getTotalIncomeByMonth(now.getFullYear(), now.getMonth() + 1)
  const balance     = incomeTotal - totalMonth

  const usageValues  = Object.values(budgetUsage)
  const avgBudgetPct = usageValues.length > 0
  ...
  const spentPct      = incomeTotal > 0 ? (totalMonth / incomeTotal) * 100 : null
  ...
  const pdfData = {
    ...
    incomeTotal, expenseTotal: totalMonth, balance,
    spentPct,
  ...
  async function handleExportPDF() {
    ...
    setGenerating(true)
    try {
      await generateMonthlyPDF(pdfRef.current, now)
    } finally {
      setGenerating(false)
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <button
          ...
          disabled={generating}
          className="flex items-center gap-2 bg-indigo-600/20 hover:bg-indigo-600/30 border border-indigo-500/30 text-indigo-400 text-xs font-medium px-3 py-2 rounded-lg transition-colors disabled:opacity-50 flex-shrink-0"
        >
          <FileDown size={14} />
      ...
      {/* Fila 1: Stat cards (5 tarjetas) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4">
        <StatCard
          icon={DollarSign}
          label="Total gastado este mes"
          value={formatCurrency(totalMonth)}
          sub={spentPct !== null ? `${spentPct.toFixed(0)}% del ingreso` : undefined}
        ...
        <StatCard
          ...
          sub={balance >= 0 ? 'A favor' : 'En déficit'}
          iconBg={balance >= 0 ? 'bg-emerald-500/15' : 'bg-red-500/15'}
          iconText={balance >= 0 ? 'text-emerald-400' : 'text-red-400'}
      ...
      {createPortal(
        <PDFReportContent ref={pdfRef} data={pdfData} />,
        document.body
      )}
    </div>
  )
